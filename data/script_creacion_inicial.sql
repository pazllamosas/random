...
IF OBJECT_ID('RANDOM.VALIDACION_TURNOS_POR_AFILIADO') IS NOT NULL
 DROP FUNCTION RANDOM.VALIDACION_TURNOS_POR_AFILIADO
...
BEGIN 
	...
	VALUES (@NOMBRE, @APELLIDO, @SEXO, @IDTIPODOC, @DOCUMENTO, @DIRECCION, @TELEFONO, @MAIL, convert (datetime2,@FECHANAC))
...
BEGIN 
	UPDATE RANDOM.PERSONA SET
		...
		Telefono = @TELEFONO,
		Mail = @MAIL,
		Fecha_Nac = convert (datetime2,@FECHANAC)
...
BEGIN
	...
	INSERT INTO @TABLA_DE_DIAS VALUES ('viernes', 6)
...
CREATE FUNCTION RANDOM.VALIDACION_TURNOS_POR_AFILIADO(@Afiliado int, @Año int, @Mes int, @Dia int, @Hora int, @Minutos INT)
RETURNS INT
AS BEGIN
...
IF(0 =
(SELECT COUNT(*) FROM RANDOM.TURNO A WHERE A.IdAfiliado = @id AND year(a.FechaYHoraTurno) = @año and month(a.FechaYHoraTurno) = @Mes
and Day(a.FechaYHoraTurno) = @Dia and DATEPART(hour,a.FechaYHoraTurno) = @Hora and DATEPART(MINUTE,a.FechaYHoraTurno) = @Minutos)
)
BEGIN
SET @RTA = 1
END
