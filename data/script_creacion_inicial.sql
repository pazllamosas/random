...
BEGIN 
	...
	INSERT INTO RANDOM.AFILIADO (IdPersona,NumeroAfiliadoRaiz )
	VALUES((SELECT P.IdPersona FROM RANDOM.PERSONA P WHERE P.Documento = @DOCUMENTO), @NRO_AFILIADO_RAIZ)
END 
GO
...
CREATE PROCEDURE RANDOM.NRO_AFILIADO_FAMILIARES (@NROAFILIADORAIZ nvarchar) AS
BEGIN 
...
IF ((@NRO_AFILIADO_NUMERICO = '01') OR (@NRO_AFILIADO_NUMERICO = '02'))
	...
	WHERE NumeroAfiliadoRaiz = @NROAFILIADORAIZ
ELSE 
	UPDATE RANDOM.AFILIADO SET
	NumeroAfiliadoExt = RANDOM.SIGUIENTE_AFILIADO ()
	WHERE NumeroAfiliadoRaiz = @NROAFILIADORAIZ
END 
GO

CREATE PROCEDURE RANDOM.NRO_AFILIADO_CONYUGE (@NROAFILIADORAIZ nvarchar) AS
BEGIN
...
GO
CREATE PROCEDURE RANDOM.BUSCAR_MEDICO(@Descripcion nvarchar(255), @Apellido nvarchar(255)) AS
BEGIN
IF(@Descripcion = '')
  BEGIN
