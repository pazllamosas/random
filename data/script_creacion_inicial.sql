...
BEGIN 
	UPDATE RANDOM.ROL SET
		Estado = 0
	WHERE IdRol = @ROL
	DELETE FROM RANDOM.USUARIO_POR_ROL
		WHERE IdRol = @ROL
END 
GO
...
CREATE PROCEDURE RANDOM.ASIGNAR_FUNCIONALIDAD(@FUNC int, @ROL int)
AS BEGIN
...
CREATE PROCEDURE RANDOM.ELIMINAR_FUNCIONALIDAD_DEL_ROL (@FUNC INT, @ROL INT) AS
BEGIN
