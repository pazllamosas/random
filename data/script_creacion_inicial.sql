...
--IF OBJECT_ID('RANDOM_NUMERO_AFILIADO_RAIZ') IS NOT NULL
--DROP PROCEDURE RANDOM_NUMERO_AFILIADO_RAIZ
...
--CREATE TRIGGER RANDOM.AFTER_NUEVO_AFILIADO ON RANDOM.AFILIADO AFTER UPDATE AS
--BEGIN
--	UPDATE RANDOM.AFILIADO SET
--	NumeroAfiliadoExt = RIGHT (('0' + NumeroAfiliadoExt), 2)
...
CREATE PROCEDURE RANDOM.CAMBIO_PLAN(@NRO_AFILIADO_RAIZ nvarchar(255),
									@IDPLAN int,
									@MOTIVO nvarchar(255))AS
BEGIN
	UPDATE RANDOM.AFILIADO SET
		IdPlan = @IDPLAN
	WHERE NumeroAfiliadoRaiz = @NRO_AFILIADO_RAIZ
	
	DECLARE @IDPERSONA INT
	SELECT @IDPERSONA = IdPersona FROM RANDOM.AFILIADO 
	WHERE @NRO_AFILIADO_RAIZ = NumeroAfiliadoRaiz
...
CREATE PROCEDURE RANDOM.NRO_AFILIADO_FAMILIARES (@NROAFILIADORAIZ nvarchar, @DOCUMENTO numeric(18,0), @CANTIDAD_A_CARGO int) AS
BEGIN 
...
CREATE PROCEDURE RANDOM.NRO_AFILIADO_CONYUGE (@NROAFILIADORAIZ nvarchar, @DOCUMENTO numeric(18,0)) AS
BEGIN
IF (( NOT EXISTS ( SELECT * FROM RANDOM.AFILIADO WHERE NumeroAfiliadoRaiz = @NROAFILIADORAIZ AND NumeroAfiliadoExt = '02')))
...
CREATE PROCEDURE RANDOM.CANCELAR_TURNO_PROFESIONAL(@PROFESIONAL int, @FECHADESDE datetime, @FECHAHASTA datetime, @TIPO int, @MOTIVO nvarchar(255)) AS
BEGIN
